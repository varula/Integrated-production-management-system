...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = `${SHEETS_API}/${SHEET_ID}/values/hourly_production!A:Z?key=${API_KEY}`
    ...
    console.error('[v0] Error fetching hourly production:', error)
    return NextResponse.json({ error: 'Failed to fetch hourly production' }, { status: 500 })
  }
}
