...
export async function GET(request: NextRequest) {
  ...
  try {
    const url = `${SHEETS_API}/${SHEET_ID}/values/production_plans!A:Z?key=${API_KEY}`
    ...
    const filtered = data.filter((plan: any) => plan.factory_id === factoryId)

    return NextResponse.json({ data: filtered })
  } catch (error) {
    console.error('[v0] Error fetching production plans:', error)
