...
export default function HourlyEntryPage() {
  ...
  const { lines } = useLinesData(factory?.id)
  const { plans: productionPlans } = useProductionPlansData(factory?.id)
          ...
          <div className="grid lg:grid-cols-2 gap-4 mb-6">
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              >
                ...
                {sewingLines.map((l: any) => (
                  <option key={l.id} value={l.id}>
                    {l.line_name} ({l.operator_count} operators)
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              <label className="text-sm font-semibold text-foreground mb-3 block">Select Production Order</label>
              <select
                value={selectedPlan}
